								...
								<div id="major-publishing-actions">
									...
									<div id="publishing-action">
										...
										if ($job->job_status == 'approved') {
											...
											?>
											<input name="mg_approve" type="submit" class="button-primary" id="mg_approve" style="display: inline;" value="<?php _e('Approve'); ?>">
											<br/><br/>
						...
						<div class="stuffbox">
							...
							<div class="inside">
								...
								<p>
									...
									<label for="mg_public" class="selectit"><input id="mg_public" name="mg_public" type="checkbox" value="Y" <?php if ($mg_public) { ?>checked<?php } ?>> <?php _e('Yes, you can use this translation as a public example of myGengo\'s service'); ?>.</label>
								</p>
							</div>
						</div>
						<?php
...
function job_comment_meta_box($job) {
	...
	<div id="commentsdiv" class="postbox">
		...
		<h3 class="hndle"><span><?php _e('Comments and Activity'); ?></span></h3>
		<div class="inside">
			<?php
			if ( 1 > $total ) {
			...
			if(!($job->job_status == 'approved' || $job->job_status == 'cancelled')) {
				?>
				<p><?php _e('Add a comment for the translator'); ?>:</p><br />
				<p><textarea rows="1" cols="40" name="mg_body_comment" style="height: 4em; margin: 0px; width: 98%;"></textarea></p>
				<p><?php _e('Requesting personal contact details from translators is prohibited'); ?>.</p>
				<p><input name="mg_comment" type="button" class="button" id="mg_comment" style="display: inline;" value="<?php _e('Add Comment'); ?>" onclick="document.forms.review.action='<?php echo mygengo_getLink($wp_admin_url.'/admin.php',array('mg_add_comment' => '1'),true);?>';document.forms.review.submit();"></p>
				<?php
				}
			?>
		</div>
